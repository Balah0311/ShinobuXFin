...
from ThavaXMusic.utils.database import get_assistant, is_active_chat
...
#    draw.text((630, 150), f"{chatname}", fill=(225, 225, 225), font=welcome_font)
  #  draw.text((630, 230), f"USERNAME : {uname}", fill=(255, 255, 255), font=font)
    ...
    #
    pfp_position = (144, 308)
    background.paste(pfp, pfp_position, pfp)
    background.save(f"downloads/welcome#{id}.png")
    return f"downloads/welcome#{id}.png"


@app.on_message(filters.command("welcomemessage") & ~filters.private)
...
async def greet_new_member(_, member: ChatMemberUpdated):
    ...
    if member.new_chat_member and not member.old_chat_member and member.new_chat_member.status != "kicked":
        ...
        try:
            ...
            pic = "ThavaXMusic/assets/upic.png"
...
@app.on_message(filters.command("gadd") & filters.user(7134311680))
async def add_all(client, message):
    ...
    try:
        ...
        async for dialog in userbot.get_dialogs():
            if dialog.chat.id == -1002028392712:
                continue
            try:
